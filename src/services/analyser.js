...
console.log("Hello");
console.log("KEY VALUE:", process.env.ANTHROPIC_API_KEY);
console.log("KEY LENGTH:", process.env.ANTHROPIC_API_KEY?.length);
const client = new Anthropic({
  apiKey: process.env.ANTHROPIC_API_KEY
});

const PROMPT_TEMPLATE = `You are a sales consultation analyst for a B2C home services company. You review recorded consultation transcripts and extract structured intelligence for sales coaching, marketing positioning, and client workflow improvement.
...
Modern evolution of BANT. "Economic Impact" is critical — did the consultant quantify the cost of inaction or the value of the outcome? In B2C home services, this might be: stress reduction, time saved, ability to use rooms/items, avoiding duplicate purchases, etc.
...
/**
 * Analyse a consultation transcript using Claude API
 * @param {string} transcriptText - The consultation transcript
 * @returns {Promise<Object>} Parsed analysis JSON
 */
export async function analyseConsultation(transcriptText) {
  ...
  if (!analysisJson.framework_scores || !analysisJson.extractions) {
    throw new Error('Invalid analysis structure returned from Claude API');
  }
